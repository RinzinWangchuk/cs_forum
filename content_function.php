<?php
	...
	function dispcategories() {
		...
		while ($row = mysqli_fetch_assoc($select)) {
			...
			echo "<tr><td class='main-category' colspan='2'><b>".$row['category_title']."</b></td></tr>";
	...
	function dispsubcategories($parent_id) {
		...
		while ($row = mysqli_fetch_assoc($select)) {
			echo "<tr><td class='category_title'><a href='/cs_forum/topics.php?cid=".$row['cat_id']."&scid=".$row['subcat_id']."'>
			<b>".$row['subcategory_title']."</b><br />";
			echo "<p style='margin-left:7px'>".$row['subcategory_desc']."</p></a></td>";
	...
	function disptopics($cid, $scid) {
		...
		if ($count != 0) {
			...
			while ($row = mysqli_fetch_assoc($select)) {
				echo "<tr><td><a href='/cs_forum/read_topic.php?cid=".$cid."&scid=".$scid."&tid=".$row['topic_id']."'>
					 ".$row['title']."</a></td><td>".$row['author']."</td><td>".$row['date_posted']."</td><td>".$row['views']."</td>
					 <td>".$row['replies']."</td></tr>";
			}
			echo "</table>";
		} else {
			echo "<p>this category has no topics yet!  <a href='/cs_forum/new_topic.php?cid=".$cid."&scid=".$scid."'>
	...
	function addview($cid, $scid, $tid) {
		...
		function replylink($cid, $scid, $tid) {
			echo "<p><a href='/cs_forum/reply_to.php?cid=".$cid."&scid=".$scid."&tid=".$tid."'>Reply to this post</a></p>";
		}

		function replytopost($cid, $scid, $tid) {
			echo "<div class='content'><form action='/cs_forum/add_reply.php?cid=".$cid."&scid=".$scid."&tid=".$tid."' method='POST' name='reply'>
				  <p>Reply: </p>
					<div id='reply_comment_errorloc' style='color:red; font-size:12px'></div>
				  <textarea cols='80' rows='5' id='comment' name='comment'></textarea><br />
					<input type='submit' value='add reply' />
				  </form>
						<script>
							var val = new Validator('reply');
							val.addValidation('comment', 'req', 'You cannot leave it empty!');
							val.addValidation('comment', 'minlen=10', 'minimum length shoud be 10');
